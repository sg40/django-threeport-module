...
import (
	"context"
	django "django-threeport-module/internal/django"
...
func main() {
	// flags
	var djangoDefinitionConcurrentReconciles = flag.Int(
		"django-definition-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for django definitions",
	)
	var djangoInstanceConcurrentReconciles = flag.Int(
		"django-instance-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for django instances",
	)
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      django.LockBucketName,
		Description: django.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", django.LockBucketName)
		os.Exit(1)
	}

	// wait for notification stream to be available before proceeding
	controller.WaitForStream(js, notif.DjangoStreamName, log)
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *djangoDefinitionConcurrentReconciles,
		Name:                 "DjangoDefinitionReconciler",
		NotifSubject:         notif.DjangoDefinitionSubject,
		ReconcileFunc:        django.DjangoDefinitionReconciler,
	})
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *djangoInstanceConcurrentReconciles,
		Name:                 "DjangoInstanceReconciler",
		NotifSubject:         notif.DjangoInstanceSubject,
		ReconcileFunc:        django.DjangoInstanceReconciler,
	})
	...
	for _, r := range reconcilerConfigs {
		...
		_, err = js.AddConsumer(notif.DjangoStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.DjangoStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "DjangoController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info(
		"django controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", django.LockBucketName,
	)
	...
	log.Info("django controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-django-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
